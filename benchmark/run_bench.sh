...
# go build -o ${out_dir}/client $jenn_dir/grpc-go/examples/helloworld/greeter_client/client.go
/usr/local/go/bin/go build -o ${out_dir}/client $jenn_dir/grpc-go/benchmark/client/main.go
if [ $? != 0 ]; then
  clean_and_die 1
fi
