...
import (
	...
	testpb "google.golang.org/grpc/examples/helloworld/helloworld"
	"google.golang.org/grpc/grpclog"
	"google.golang.org/grpc/internal/syscall"
)

var (
	addr	  = flag.String("addr", "localhost", "address of server node.")
...
func runWithConn(cc *grpc.ClientConn, req *testpb.HelloRequest, warmDeadline, endDeadline time.Time) {
	for i := 0; i < *numRPC; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
...
func makeCaller(cc *grpc.ClientConn, req *testpb.HelloRequest) func() {
	//client := testpb.NewBenchmarkServiceClient(cc)
	client := testpb.NewGreeterClient(cc)
	//if *rpcType == "unary" {
	return func() {
